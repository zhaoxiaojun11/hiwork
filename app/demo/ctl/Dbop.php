 ...
 * chy 20190926161151
 */
class Dbop extends \kernel\Controler
{
	public function index()
	{
		rtn::alert('此模块需数据库支持，未配置数据库则本模块所有方法不能用');
	}

	public function select()
	{
		$r = db::mine()->R('select * from t_myh_user');
		var_dump($r);
	}
	...
	{
		$r = db::mine()->R('select * from t_myh_lishi where m=? and d=? and stat>0 order by id desc', [date('n'), date('j')]);

		$r===false ? Rtn::err('系统错误：'.db::mine()->getErr(), 0, 500) : Rtn::okk('ok', $r);
	}

	public function insert()
	{
		$c = db::mine()->I('t_myh_lishi', [
		...
		if($c===false)
		{
			Rtn::err('系统错误：'.db::mine()->getErr(), 0, 500);
		}
		else
		{
			// 取最后生成的ID
	...
	public function delete()
	{
		...
		if($lastid)
		{
			...
			//删除数据
			$c=db::mine()->exec('delete from t_myh_lishi where id=?', [$lastid]);
			if($c===false)
			{
				rtn::err('系统错误：'.db::mine()->getErr(), 0, 500);
			}
